...
const PORTFOLIO = [PROJECT_A, PROJECT_B, PROJECT_C];
...
function buildReport({ projects, results, dashboard }) {
  ...
  lines.push(`> 3-project portfolio executed live against \`https://demoqa.com\`.`);
  ...
  lines.push(`## 🔁 QA iteration journey (2 build cycles)`);
  lines.push('');
  lines.push(`A real QA build is rarely green on the first try. This portfolio`);
  lines.push(`went through **two iterations** before sign-off — exactly how a`);
  lines.push(`QA team would use the dashboard's failure-feedback loop:`);
  lines.push('');
  lines.push(`| # | Iteration | Outcome | Triage |`);
  lines.push(`|---|---|---|---|`);
  lines.push(`| 1 | Initial recipes (selectors based on demoqa docs) | 2 / 7 passed (28.6%) | Failure Insights tab showed clusters around \`a[href="/text-box"]\` — demoqa's menu has ad overlays that intercept clicks |`);
  lines.push(`| 2 | Selectors hardened — direct \`navigate\` instead of menu-click; \`#name\` instead of \`#output\` for assert | **${totalPassed} / ${totalReruns} passed (${passRate}%)** | All scenarios green — promoted to sign-off |`);
  lines.push('');
  lines.push(`Total dashboard rerun count = ${dRuns.length} (covers both iterations,`);
...
// ─────────────────────────────────────────────────────────────────
(async () => {
  log(`══ ZAC QA Portfolio — building 3 projects against ${BASE} ══`);
  // Ensure server is up
  ...
  for (const p of PORTFOLIO) {
    ...
    const runs  = [];
    if (build.ok) {
      // Run 2-3 times per project to populate trend
      const runCount = p === PROJECT_A ? 3 : 2;
